...
import org.openmrs.module.kenyaemr.fragment.controller.intake.EnterLabResultFragmentController;
...
public class PatientUtilsFragmentController {
	...
	/**
	 * Gets the patient flags for the given patient. If any of the calculations
	 * throws an exception, this will return a single flag with a message with
	 * the name of the offending calculation
	 * 
	 * @param patientId
	 *            the patient id
	 * @param calculationManager
	...
	public List<SimpleObject> getFlags(@RequestParam("patientId") Integer patientId, @SpringBean CalculationManager calculationManager) {
		...
		for (PatientFlagCalculation calc : calculationManager.getFlagCalculations()) {
			try {
				...
				// logger.info(String.format("Patient Flag Calculation class is %s",
				// calc));

				if (result != null && (Boolean) result.getValue()) {
					flags.add(SimpleObject.create("message", calc.getFlagMessage()));
				}
				// logger.info(String.format("Calucation result is %s",
				// result));
			} catch (Exception ex) {
				logger.error("Error evaluating " + calc.getClass(), ex);
	...
	private boolean getMsmConcept(){
		EncounterService encounterService = Context.getEncounterService();
		Encounter encounter = encounterService.getEncounter(29);
		Concept concept = Context.getConceptService().getConcept(160578);
