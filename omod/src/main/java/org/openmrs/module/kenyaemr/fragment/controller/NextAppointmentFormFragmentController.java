...
public class NextAppointmentFormFragmentController {
	...
	private Obs getAllLatestObs( String conceptIdentifier, Visit visit) {
		//seint added if condition  for visit empty case
		System.out.println("visit:"+visit);
		System.out.println("concept identifier"+conceptIdentifier);
		Concept concept = Dictionary.getConcept(conceptIdentifier);
		List<Obs> obs = Context.getObsService()
				.getObservationsByPersonAndConcept(visit.getPatient(), concept);
		List<Obs> currentVisitObs = new ArrayList<Obs>();
		System.out.println("current visit:"+currentVisitObs);
		System.out.println("Obs o value:"+ obs);
		//seint added if condition  for visit empty case
		if (!currentVisitObs.isEmpty())	{	
		for(Obs o : obs){
