...
<form id="enterLabResultForm" action="${ ui.actionLink("kenyaemr", "intake/enterLabResult", "submit") }" method="post">
	...
	<% listTests.eachWithIndex { test , count -> %>
		...
		<tr onclick="EnterLabResult.redirectFocus(this)" name="${answerConcept.id}">
			...
			<td name="optional-comment" ></td>
			<td></td>
		</tr>
		<% } %>
		<% } else if(test.concept.id==161471 || test.concept.id == 111759) { %>
		...
		<%} else if(test.conceptId ==  112352 ){ %>
			...
			<td class="unit" name="${test.conceptId}" value="${test.conceptId}" colspan="3" style="text-align:left;"  >
				...
				<select onchange="EnterLabResult.updateDropDown(this,${test.conceptId});" >
					<option></option>
					<option>No MTB</option>
					<option>Invalid/No result</option>
		...
		<tr name="${test.conceptId}" onclick="EnterLabResult.redirectFocus(this)" >
			...
			<td class="description">
				${ test.name }
				<input type="hidden" name="conceptIds" value="${test.conceptId}"/>
				<input type="hidden" name="conceptValue" value="${test.result}" /> 
			</td>
	...
	<div>
	<% if (!confirmed) { %>
	<input type="button" value="Confirm" onclick="EnterLabResult.confirmResult()" class="btn btn-warn" />
