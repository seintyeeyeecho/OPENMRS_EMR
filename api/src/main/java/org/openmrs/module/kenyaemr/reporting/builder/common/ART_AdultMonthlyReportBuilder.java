...
@Component
@Builds({ "kenyaemr.common.report.artAdultmonthlyReport" })
public class ART_AdultMonthlyReportBuilder extends AbstractReportBuilder {
	...
	protected List<Mapped<DataSetDefinition>> buildDataSets(ReportDescriptor descriptor, ReportDefinition report) {
		return Arrays.asList(ReportUtils.map(creatArtOutcomeDeathDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(creatArtOutcomeTransferredOutDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(creatArtOutcomeLostMissingDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(creatArtOutcomeCompletedArtDataSet(), "startDate=${startDate},endDate=${endDate}"),
				...
				ReportUtils.map(createCumulativeTbDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(createHivDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(createCumulativeHivDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(creatDataSetForCdFourCount(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(creatDataSetForViralLoad(), "startDate=${startDate},endDate=${endDate}"),
				ReportUtils.map(createNewHIVEnrolledDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(createHIVEnrolledwithPerformanceADataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(createHIVEnrolledwithRiskDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(createOIDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(createHIVAdherenceDataSet(), "startDate=${startDate},endDate=${endDate}"),
				ReportUtils.map(createLevelAdherenceDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(createElligibleARTDataSet(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(firstregimensDataset(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(secondregimensDataset(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(hivhbvregimensDataset(), "startDate=${startDate},endDate=${endDate}"),
				ReportUtils.map(originalDataset(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(onsubsituteDataset(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(onswitchsecondDataset(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(onswitchthirdDataset(), "startDate=${startDate},endDate=${endDate}"), ReportUtils.map(onstockDispensedDataset(), "startDate=${startDate},endDate=${endDate}"));
	}
	...
	private DataSetDefinition creatArtOutcomeDeathDataSet() {
		...
		EmrReportingUtils.addRow(dsd, "A1", "No. of ", ReportUtils.map(hivIndicators.died(), indParams), allColumns, indSuffixes);
		return dsd;
	}
	...
	private DataSetDefinition createCumulativeactiveTbDataSet() {
		...
		List<ColumnParameters> allColumns = Arrays.asList(female, male, total);
		List<String> indSuffixes = Arrays.asList("FM", "MA", "TT");
		// EmrReportingUtils.addRow(dsd, "F1",
		// "No. of detected cases (Cumulative no. of active follow up  patients ever started on ART )",
		// ReportUtils.map(artIndicators.startedArtCumulative(), indParams),
		// allColumns, indSuffixes);
		EmrReportingUtils.addRow(dsd, "F1", "No. of detected cases (Cumulative no. of active follow up  patients ever started on ART )", ReportUtils.map(artIndicators.activeFollowUpOnART(), indParams), allColumns, indSuffixes);
	...
	private DataSetDefinition createTbDataSet() {
		...
		EmrReportingUtils.addRow(dsd, "H1", "No. of detected cases (Transferred in)", ReportUtils.map(hivIndicators.transferIn(), indParams), allColumns, indSuffixes);
		return dsd;
	}
	...
	private DataSetDefinition createHIVEnrolledwithRiskDataSet() {
		...
		MSM.add(new ColumnParameters("TT", "Total MSM Risk Factor", "age=15+|parameter=1"));
