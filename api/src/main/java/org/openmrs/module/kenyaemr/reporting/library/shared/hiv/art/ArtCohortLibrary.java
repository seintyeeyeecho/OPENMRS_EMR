...
public class ArtCohortLibrary {
	...
	/**
	 * Patients who are in the "month net cohort" on ${onDate} Patients who
	 * started art between dates given months Used for art cohort analysis
	 * 
	 * @return the cohort definition
	 */
	...
	/**
	 * Patients on the given regimen. In the future this should look at
	 * dispensing records during the reporting period which implicitly check
	 * whether a patient is active. As a workaround until we get to dispensing
	 * records, we explicitly check whether a patient is active here by looking
	 * for recent encounters.
	 *
	 * @return the cohort definition
	 */
	...
	public CohortDefinition startsART() {
		...
		// cd.addSearch("completehivProgram",
		// ReportUtils.map(commonCohorts.compltedProgram(MetadataUtils.existing(Program.class,
		// Metadata.Program.HIV)), "completedOnOrBefore=${onOrBefore}"));
	...
	public CohortDefinition cumulativeActiveFollowUp() {
		CompositionCohortDefinition cd = new CompositionCohortDefinition();
		cd.setName("Started ART excluding transfer ins on date in this facility");
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		//cd.addParameter(new Parameter("calendarStartDate", "Calendar year start Date", Date.class));
		cd.addSearch("startedArt", ReportUtils.map(startsART(), "onOrBefore=${onOrAfter-1d}"));
		cd.addSearch("artOutCome", ReportUtils.map(commonCohorts.artOutComePatient(), "onOrBefore=${onOrBefore-1d}"));
		cd.addSearch("deceased", ReportUtils.map(commonCohorts.deceasedPatients(), "onOrBefore=${onOrBefore-1d}"));
		//cd.addSearch("completeProgram", ReportUtils.map(commonCohorts.compltedProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "completedOnOrAfter=${onOrBefore},completedOnOrBefore=${onOrAfter-1d}"));
		cd.addSearch("completeProgram", ReportUtils.map(commonCohorts.compltedProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "completedOnOrBefore=${onOrAfter-1d}"));
		cd.setCompositionString("startedArt AND NOT (deceased OR completeProgram OR artOutCome)");
		return cd;
	}
	...
	public CohortDefinition startedArtExcludingTransferinsOnDates() {
		...
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		// cd.addSearch("startedArt", ReportUtils.map( startsART(),
	 ...
	 * cd.addParameter(new Parameter("completedBefore", "Complete Date",
	 * Date.class));
	 * 
	 * cd.addSearch("completedProgram",
	 * ReportUtils.map(commonCohorts.compltedProgram
	 * (MetadataUtils.existing(Program.class, Metadata.Program.ART)),
	 * "completedOnOrBefore=${onOrBefore}"));
	 * cd.setCompositionString("completedProgram"); return cd; }
	 */

	/**
	 * Patients who started ART while pregnant between ${onOrAfter} and
	 * ${onOrBefore}
	 * 
	...
	/**
	 * Patients who started ART while being a TB patient between ${onOrAfter}
	 * and ${onOrBefore}
	 * 
	 * @return the cohort definition
	 */
	...
	/**
	 * Patients who started ART with the given WHO stage between ${onOrAfter}
	 * and ${onOrBefore}
	 * 
	 * @return the cohort definition
	 */
