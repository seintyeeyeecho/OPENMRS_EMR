...
import org.openmrs.module.kenyaemr.calculation.library.hiv.AZT3TCplusNVPCalculation;
...
import org.openmrs.module.kenyaemr.calculation.library.hiv.FirstLineCalculation;
...
import org.openmrs.module.kenyaemr.calculation.library.hiv.LastOutcomeLossCalculation;
import org.openmrs.module.kenyaemr.calculation.library.hiv.LastOutcomeResultCalculation;
import org.openmrs.module.kenyaemr.calculation.library.hiv.LastOutcomeStopArtCalculation;
import org.openmrs.module.kenyaemr.calculation.library.hiv.LastOutcomeTransferCalculation;
...
public class HivCohortLibrary {
	...
	/**
	 * Patients who were enrolled in HIV care (including transfers) between
	 * ${onOrAfter} and ${onOrBefore}
	 * 
	 * @return the cohort definition
	 */
	...
	/**
	 * Patients who were enrolled in HIV care (excluding transfers) between
	 * ${onOrAfter} and ${onOrBefore}
	 * 
	 * @return the cohort definition
	 */
	...
	/**
	 * Patients who were enrolled in HIV care (excluding transfers) from the
	 * given entry points between ${onOrAfter} and ${onOrBefore}
	 * 
	 * @return the cohort definition
	 */
	...
	/**
	 * Patients who were enrolled in HIV care (excluding transfers) not from the
	 * given entry points between ${onOrAfter} and ${onOrBefore}
	 * 
	 * @return the cohort definition
	 */
	...
	public CohortDefinition stoppArt() {
		CalculationCohortDefinition last = new CalculationCohortDefinition(new LastOutcomeStopArtCalculation());
		last.setName("last outcome of a  patients on date");
		last.addParameter(new Parameter("onDate", "On Date", Date.class));
		CompositionCohortDefinition cd = new CompositionCohortDefinition();
		cd.setName("Started ART excluding transfer ins on date in this facility");
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.addSearch("startedArt", ReportUtils.map(artCohorts.startedArtExcludingTransferinsOnDates(), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		...
		cd.addSearch("last", ReportUtils.map(last, "onDate=${onOrBefore}"));
		cd.setCompositionString("last AND startedArt AND completeProgram");
		return cd;
	}
	...
	public CohortDefinition restartedProgram() {
		...
		// cd.addSearch("completeProgram",
		// ReportUtils.map(commonCohorts.compltedProgram(MetadataUtils.existing(Program.class,
		// Metadata.Program.ART)), "completedOnOrBefore=${onOrBefore}"));
	...
	public CohortDefinition transferIn() {
		Concept entrypoint = Dictionary.getConcept(Dictionary.METHOD_OF_ENROLLMENT);
		Concept transferInArt = Dictionary.getConcept(Dictionary.PATIENT_TRANSFERED_ON_ART);
		Concept preArt = Dictionary.getConcept(Dictionary.PATIENT_TRANSFERED_PRE_ART);
		CompositionCohortDefinition cd = new CompositionCohortDefinition();
		cd.setName("Transfer In...");
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));

		cd.addSearch("entrypointOne", ReportUtils.map(commonCohorts.hasObs(entrypoint, transferInArt), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		//cd.addSearch("preart", ReportUtils.map(commonCohorts.hasObs(entrypoint, preArt), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		
		// cd.addSearch("completeProgram",
		// ReportUtils.map(commonCohorts.compltedProgram(MetadataUtils.existing(Program.class,
		// Metadata.Program.ART)), "completedOnOrBefore=${onOrBefore}"));
		cd.setCompositionString("(entrypointOne)");
		return cd;
	}
	...
	public CohortDefinition died() {
		...
		comp.addSearch("deceased",ReportUtils.map(commonCohorts.deceasedPatients(),"onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		
		CompositionCohortDefinition onArtDefinition = new CompositionCohortDefinition();
		onArtDefinition.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		onArtDefinition.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		comp.addSearch("enrolledOnArt", ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "enrolledOnOrBefore=${onOrBefore}"));
		
		comp.setCompositionString("deceased AND enrolledOnArt");
		return comp;
	}
	...
	public CohortDefinition reasonOfoutcome() {
		...
		CalculationCohortDefinition last = new CalculationCohortDefinition(new LastOutcomeResultCalculation());
		last.setName("last outcome of a  patients on date");
		last.addParameter(new Parameter("onDate", "On Date", Date.class));
		...
		// ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class,
		// Metadata.Program.ART)),"enrolledOnOrAfter=${onOrAfter},enrolledOnOrBefore=${onOrBefore}"));
		...
		comp.addSearch("stop", ReportUtils.map(stoppArt(), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		comp.addSearch("deceased", ReportUtils.map(cd, "onDate=${onOrBefore}"));
		comp.addSearch("last", ReportUtils.map(last, "onDate=${onOrBefore}"));
		comp.setCompositionString("deceased AND outcomeReason AND last AND NOT stop");
		return comp;
	}

	public CohortDefinition reasonOfoutcometransfer() {
		CalculationCohortDefinition last = new CalculationCohortDefinition(new LastOutcomeTransferCalculation());
		last.setName("last outcome of a  patients on date");
		last.addParameter(new Parameter("onDate", "On Date", Date.class));
		...
		// ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class,
		// Metadata.Program.ART)),"enrolledOnOrAfter=${onOrAfter},enrolledOnOrBefore=${onOrBefore}"));
		// cd.addSearch("outcomeReason",
		// ReportUtils.map(commonCohorts.hasObs(outcome ,reason),
		// "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.addSearch("outcomeReason", ReportUtils.map(artCohorts.startedArtExcludingTransferinsOnDates(), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.addSearch("last", ReportUtils.map(last, "onDate=${onOrBefore}"));
		cd.addSearch("stop", ReportUtils.map(stoppArt(), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.setCompositionString("outcomeReason AND last AND NOT stop");
		return cd;
	}

	public CohortDefinition reasonOfoutcomeMissing() {
		CalculationCohortDefinition last = new CalculationCohortDefinition(new LastOutcomeLossCalculation());
		last.setName("last outcome of a  patients on date");
		last.addParameter(new Parameter("onDate", "On Date", Date.class));
	...
	public CohortDefinition totalArtpatient() {
		...
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		// cd.addSearch("enrolled",
		// ReportUtils.map(artCohortsstartedArtExcludingTransferinsOnDates,"enrolledOnOrAfter=${onOrAfter},enrolledOnOrBefore=${onOrBefore}"));
	...
	//	Concept tbPatientsstatus = Dictionary.getConcept(Dictionary.TB_Rx);
		Concept tbPatientsstatus = Dictionary.getConcept(Dictionary.YES);
		CompositionCohortDefinition cd = new CompositionCohortDefinition();
		...
		//cd.addSearch("tbPatient", ReportUtils.map(commonCohorts.hasObs(tbPatients, tbPatientsstatus), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		//cd.addSearch("enrolledInArt", ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "enrolledOnOrAfter=${onOrAfter},enrolledOnOrBefore=${onOrBefore}"));
		//cd.addSearch("completeArtProgram", ReportUtils.map(commonCohorts.compltedProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "completedOnOrBefore=${onOrBefore}"));
		//cd.addSearch("completedProgram", ReportUtils.map(commonCohorts.compltedProgram(), "completedOnOrBefore=${onOrBefore}"));
		//cd.setCompositionString("tbPatient AND enrolledInArt AND NOT (completeArtProgram) AND NOT (dead OR transferout OR lostmissing)");

		cd.addSearch("completeArtProgram", ReportUtils.map(commonCohorts.compltedProgram(), "completedOnOrBefore=${onOrBefore}"));
        cd.addSearch("oldTbPatient", ReportUtils.map(commonCohorts.hasObs(tbPatients, tbPatientsstatus), "onOrBefore=${onOrBefore}"));
        cd.addSearch("newTbPatient", ReportUtils.map(commonCohorts.hasObs(tbPatients, tbPatientsstatus), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
        cd.addSearch("newEnrolledInArt", ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "enrolledOnOrAfter=${onOrAfter},enrolledOnOrBefore=${onOrBefore}"));
        cd.addSearch("oldEnrolledInArt", ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "enrolledOnOrBefore=${onOrBefore}"));
        cd.setCompositionString(" ((oldTbPatient AND newEnrolledInArt) OR (newTbPatient AND oldEnrolledInArt))  AND NOT (completeArtProgram) AND NOT (dead OR transferout OR lostmissing)");
	...
	public CohortDefinition initializedHIV() {
		...
		CompositionCohortDefinition cd = new CompositionCohortDefinition();
		cd.setName("initalized list for ART");
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
	...
	public CohortDefinition notinitializedART() {
		...
		cd.addSearch("enrolledHIV", ReportUtils.map(initializedHIV(), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.addSearch("completeProgram", ReportUtils.map(commonCohorts.compltedProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "completedOnOrBefore=${onOrBefore}"));
		// cd.addSearch("enrolledHIV",
		// ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class,
		// Metadata.Program.HIV)),"enrolledOnOrAfter=${onOrAfter},enrolledOnOrBefore=${onOrBefore}"));
		cd.setCompositionString("onwaitinglistArt AND enrolledHIV AND NOT completeProgram");
		return cd;
	}
	...
	public CohortDefinition riskFactorsMSM() {
		Concept risk = Dictionary.getConcept(Dictionary.HIV_RISK_FACTOR);
		Concept msm = Dictionary.getConcept(Dictionary.RISK_FACTOR_MSM);

		CompositionCohortDefinition cd = new CompositionCohortDefinition();
		cd.setName("patient with risk 1(MSM). ");
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addSearch("msm", ReportUtils.map(commonCohorts.hasObs(risk, msm), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.addSearch("inArt", ReportUtils.map(commonCohorts.inProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "onDate=${onOrBefore}"));
		cd.setCompositionString("msm AND inArt");
		return cd;
	}

	public CohortDefinition sexWorker() {
		Concept risk = Dictionary.getConcept(Dictionary.HIV_RISK_FACTOR);
		Concept sexWorker = Dictionary.getConcept(Dictionary.RISK_FACTOR_SW);

		CompositionCohortDefinition cd = new CompositionCohortDefinition();
		cd.setName("patient with risk 2");
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		cd.addSearch("sexWorker", ReportUtils.map(commonCohorts.hasObs(risk, sexWorker), "onOrAfter=${onOrAfter},onOrBefore=${onOrBefore}"));
		cd.addSearch("inArt", ReportUtils.map(commonCohorts.enrolled(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "enrolledOnOrAfter=${onOrAfter},enrolledOnOrBefore=${onOrBefore}"));
		cd.setCompositionString("sexWorker AND inArt");
		return cd;
	}

	public CohortDefinition hetroSexual() {
		Concept risk = Dictionary.getConcept(Dictionary.HIV_RISK_FACTOR);
		...
		cd.addSearch("inArt", ReportUtils.map(commonCohorts.inProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "onDate=${onOrBefore}"));
		cd.setCompositionString("risk3 AND inArt");
		return cd;
	}
	...
	public CohortDefinition riskFactors4() {
		...
		cd.addSearch("inArt", ReportUtils.map(commonCohorts.inProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "onDate=${onOrBefore}"));
		cd.setCompositionString("risk4 AND inArt ");
		return cd;
	}

	public CohortDefinition bloodTrans() {
		Concept risk = Dictionary.getConcept(Dictionary.HIV_RISK_FACTOR);
		...
		cd.addSearch("inArt", ReportUtils.map(commonCohorts.inProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "onDate=${onOrBefore}"));
		cd.setCompositionString(" risk5 AND inArt");
		return cd;
	}

	public CohortDefinition riskFactorsMotherToChild() {
		Concept risk = Dictionary.getConcept(Dictionary.HIV_RISK_FACTOR);
		...
		cd.addSearch("inArt", ReportUtils.map(commonCohorts.inProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "onDate=${onOrBefore}"));
		cd.setCompositionString(" risk6 and inArt ");
		return cd;
	}
	...
	public CohortDefinition riskFactors7() {
		...
		cd.addSearch("inArt", ReportUtils.map(commonCohorts.inProgram(MetadataUtils.existing(Program.class, Metadata.Program.ART)), "onDate=${onOrBefore}"));
		cd.setCompositionString(" risk7 AND inArt ");
		return cd;
	}
	...
	public CohortDefinition onCtxProphylaxis() {
		...
		// we need to include those patients who have either ctx in the med
		// orders
	...
	/**
	 * Patients who are in HIV care and are taking CTX prophylaxis between
	 * ${onOrAfter} and ${onOrBefore}
	 * 
	 * @return
	 */
	...
	/**
	 * Patients who are in HIV care and are taking Fluconazole prophylaxis
	...
	/**
	 * Patients tested for HIV and turn to be positive ${onOrAfter} and
	 * ${onOrBefore}
	 * 
	 * @return the cohort definition
	 */
